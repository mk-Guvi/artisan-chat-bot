...
export interface ChatViewDataI {
  loading: boolean;
  chats: ChatI[];
  chatId: string;
  page: number;
...
interface ChatBotState {
  ...
  chatView: {
    chats: Record<string, ChatViewDataI>;
    currentChat: string;
    context: "ONBOARDING";
  };
...
type Action =
  ...
  | {
      ...
      payload: { chatId: string; data: Partial<ChatViewDataI> };
    }
  | { type: "SET_CURRENT_CHAT"; payload: string }
  | { type: "SET_ACTION_DISABLE"; payload: boolean }
  | {
      type: "ROUTE_TO_CHATVIEW";
      payload: { chatId: string; chatData: Partial<ChatViewDataI> };
    }
  | {
      type: "ADD_MESSAGE";
...
const defaultChatViewState: ChatViewDataI = {
  loading: true,
  chats: [],
  chatId: "",
  page: 1,
  ...
  switch (action.type) {
    ...
    case "ADD_NEW_CHAT": {
      return {
        ...
        chatsListView: {
          ...state.chatsListView,
          chats: [...action.payload.chats, ...state.chatsListView.chats],
        },
        route: "chat-view",
        chatView: {
...
interface ChatbotContextValue extends ChatBotState {
  toggleChat: () => void;
  onBackToChats: () => void;
  handleChatViewData: (chatId: string, data: Partial<ChatViewDataI>) => void;
...
export const ChatbotProvider: React.FC<ChatbotProviderProps> = ({
  ...
  const handleChatViewData = useCallback(
    (chatId: string, data: Partial<ChatViewDataI>) => {
